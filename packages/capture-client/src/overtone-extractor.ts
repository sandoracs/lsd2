export interface OvertonePoint {
  frequency: number;
  amplitude: number; // 0–1
}

/**
 * Extract overtone amplitudes from an FFT magnitude buffer.
 * fftBuffer: output of AnalyserNode.getFloatFrequencyData() — values in dB.
 * fftSize: the analyser's fftSize property (NOT frequencyBinCount).
 */
export function extractOvertones(
  ...
  for (let n = 2; n <= maxOvertones + 1; n++) {
    ...
    // Normalize: noise floor = 0.0, 0 dB = 1.0
